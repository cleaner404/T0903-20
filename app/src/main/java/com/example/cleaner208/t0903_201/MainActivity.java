...
//        });
    ...
    //使用方法更新
    private void Update(String strWord, String strMeaning, String strSample) {
        SQLiteDatabase db = mDbHelper.getReadableDatabase();
    ...
    private ArrayList<Map<String, String>> ConvertCursor2List(Cursor c){
        ...
        while(c.moveToNext()){
            ...
            items.add(item);
            Log.i("Convert", c.getString(c.getColumnIndex(Words.Word.COLUMN_NAME_WORD))+" "
            +c.getString(c.getColumnIndex(Words.Word.COLUMN_NAME_MEANING))+" "
                ...
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        if (items.isEmpty()==false) {
                            Toast.makeText(MainActivity.this, "找到了", Toast.LENGTH_LONG).show();
                            setWordsListView(items);
                        }
                        else
